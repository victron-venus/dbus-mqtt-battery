...
function mount_overlay_fs ()
{
    ...
    if [[ ! -d "$lowerDir" ]]; then
        echo "$(date +%Y-%m-%d\ %H:%M:%S) ERROR: ${lowerDir} is not a directory" >&2
        return 1
    fi

    # Check if the path is a symlink
    if [[ -L "$lowerDir" ]]; then
        echo "$(date +%Y-%m-%d\ %H:%M:%S) ERROR: ${lowerDir} is a symlink and cannot be used" >&2
        return 1
    fi

    # Check if the path is not /
    if [[ "$lowerDir" == "/" ]]; then
        echo "$(date +%Y-%m-%d\ %H:%M:%S) ERROR: ${lowerDir} is the root directory and cannot be used" >&2
        return 1
    fi
    ...
    if ! mountpoint -q "${path}/${overlayName}/merged"; then
        ...
        if [[ $? -ne 0 ]]; then
            echo "$(date +%Y-%m-%d\ %H:%M:%S) ERROR: Could not mount overlay for ${lowerDir}" >&2
            return 1
        fi
    fi
    ...
    if ! mountpoint -q "${lowerDir}"; then
        ...
        if [[ $? -ne 0 ]]; then
            echo "$(date +%Y-%m-%d\ %H:%M:%S) ERROR: Could not mount bind overlay for ${lowerDir}" >&2
            return 1
        fi
    fi
}
